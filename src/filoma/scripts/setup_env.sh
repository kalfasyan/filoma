...
# Write or update a key in the .env file (in-memory, saved at the end).
# Keys in _ENV_MERGED take precedence over the original file.
declare -A _ENV_MERGED

load_existing_env() {
    if [ -f "$ENV_FILE" ]; then
        while IFS='=' read -r key value; do
            key=$(printf '%s' "$key" | sed -E 's/^[[:space:]]*#.*//' | sed -E 's/^[[:space:]]+|[[:space:]]+$//g')
            [ -z "$key" ] && continue
            value=$(printf '%s' "$value" | sed -E 's/^[[:space:]]*"|"[[:space:]]*$//g')
            _ENV_MERGED["$key"]="$value"
        done <"$ENV_FILE"
    fi
}
...
write_env_final() {
    local header="# Generated by filoma setup wizard ($(date +%Y-%m-%d))"
    echo "$header" >"$ENV_FILE"
    echo "" >>"$ENV_FILE"
    for key in "${!_ENV_MERGED[@]}"; do
        echo "${key}=${_ENV_MERGED[$key]}" >>"$ENV_FILE"
    done
}
...
check_existing_env() {
    if [ -f "$ENV_FILE" ]; then
        echo ""
        print_info "An existing ${ENV_FILE} file was found — keys will be merged (existing API keys are preserved)."
        echo ""
        load_existing_env
    fi
}
...
for m in data.get('models', [])[:10]:
    ...
    # Write .env
    set_env "FILOMA_FILARAKI_MODEL" "${MODEL}"
    if [ -n "$BASE_URL" ]; then
        set_env "FILOMA_FILARAKI_BASE_URL" "${BASE_URL}"
    else
        set_env "FILOMA_FILARAKI_BASE_URL" "http://localhost:11434/v1"
    fi
...
setup_mistral() {
    ...
    # Write .env
    set_env "MISTRAL_API_KEY" "${API_KEY}"
    set_env "FILOMA_FILARAKI_MODEL" "mistral:${MODEL}"
...
setup_openai() {
    ...
    # Write .env
    set_env "FILOMA_FILARAKI_BASE_URL" "https://api.openai.com/v1"
...
setup_openrouter() {
    ...
    # Write .env
    set_env "FILOMA_FILARAKI_BASE_URL" "https://openrouter.ai/api/v1"
    set_env "OPENAI_API_KEY" "${API_KEY}"
    set_env "FILOMA_FILARAKI_MODEL" "${MODEL}"
